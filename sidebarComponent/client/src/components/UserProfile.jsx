import React from 'react';
import style from './UserProfile.css';
let UserProfile = function(props) {
  return (
    <div className={style.container}>
      <img className={style.art} src={props.user.profile_pic} />
      <div className={style.user_name}>{props.user.username}</div>
      <div className={style.followers}>
        <img
          className={style['followers-icon']}
          src="data:image/svg+xml;base64,"
        />
        <span className={style['follower-count']}>
          {/* {props.user.follower_count} */} 0
        </span>
      </div>
      <div className={style.location}>{props.user.location}</div>
      <button className={style['follow-button']}>Follow</button>
    </div>
  );
};

export default UserProfile;
